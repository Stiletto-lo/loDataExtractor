...
export const exportTechData = async (techData: any[], folderPath: string) => {
	if (techData.length > 0) {
		...
		for (const tech of techData) {
			if (tech.name && tech.type) {
				if (typeMap.has(tech.type)) {
					...
					const currentTechProps = Object.entries(tech).filter(
						([_, v]) => v !== undefined && v !== null,
					).length;
		...
		console.log(
			`Reduced to ${uniqueTechData.length} unique tech entries after deduplication`,
		);
...
export const exportItemsData = async (
	...
	if (allItems.length > 0) {
		...
		const fixedItems = ingredientNameFixer.fixIngredientNames(
			itemsWithStrongboxDrops,
		);
				...
				if (fs.existsSync(existingFilePath)) {
					try {
						...
						const existingKeys = Object.keys(existingData).filter(
							(k) => existingData[k] !== undefined && existingData[k] !== null,
						);
						const newKeys = Object.keys(dataToExport).filter(
							(k) => dataToExport[k] !== undefined && dataToExport[k] !== null,
						);
						...
						console.error(
							`Error reading existing file ${existingFilePath}:`,
							e,
						);
					}
				}
