...
const parseLootTemplate = (filePath) => {
  ...
  if (!classInfo || !templateData || !templateData.Properties || !templateData.Properties.Loot) {
  ...
  // Process loot tables
  if (templateData.Properties.Loot.Tables && Array.isArray(templateData.Properties.Loot.Tables)) {
  ...
  const tier = extractTier(filePath) || extractTier(lootTemplate.name) || extractTier(lootTemplate.type) || "T1";
