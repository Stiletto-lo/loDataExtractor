...
export const parseVehicleData = (filePath: string) => {
	try {
		...
		const findCarryCapacity = (obj) => {
			...
			if (
				obj.Properties &&
				obj.Properties.AttachedWeightMinPenalty !== undefined
			) {
			...
			if (obj.ChildProperties && Array.isArray(obj.ChildProperties)) {
				for (const prop of obj.ChildProperties) {
					if (
						prop.Name === "AttachedWeightMinPenalty" &&
