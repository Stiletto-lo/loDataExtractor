 ...
 */
const fixIngredientNames = (items) => {
  ...
  if (!glossary || Object.keys(glossary).length === 0) {
    console.warn("Item name glossary is empty, ingredient names will not be fixed");
    return items;
  }

  console.log(`Applying name glossary to ingredient names (${Object.keys(glossary).length} entries available)`);
  let fixedCount = 0;
