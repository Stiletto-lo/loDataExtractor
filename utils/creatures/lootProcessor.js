	...
	if (creature.lootTable) {
		...
		if (!dataTable) {
			...
			try {
				...
				const exportDir = path.join(process.cwd(), 'exported', 'loot_templates');
				...
				const searchLootTemplate = (dir, lootTableName) => {
					...
					for (const file of files) {
						...
						} else if (file.name.toLowerCase().includes(lootTableName.toLowerCase()) && file.name.endsWith('.json')) {
							return filePath;
						}
					}
				...
				if (lootTemplateFile) {
					...
					const lootTemplateData = JSON.parse(fs.readFileSync(lootTemplateFile, 'utf8'));
					
					// Add drops information to the creature
					if (lootTemplateData && lootTemplateData.tables && lootTemplateData.tables.length > 0) {
							...
							if (table.drops && Array.isArray(table.drops)) {
								...
								for (const drop of table.drops) {
									if (drop.name && drop.chance) {
										...
										const effectiveChance = (drop.chance * tableRunChance / 100).toFixed(4);
										
										// Create drop entry
										const dropEntry = {
											name: drop.name,
											chance: parseFloat(effectiveChance),
											minQuantity: drop.minQuantity || 1,
											maxQuantity: drop.maxQuantity || 1
										};
										
										// Check if this item already exists in the drops array
										const existingDropIndex = creature.drops.findIndex(d => d.name === drop.name);
										...
										if (existingDropIndex >= 0) {
											...
											existingDrop.chance = Math.max(existingDrop.chance, dropEntry.chance);
											existingDrop.minQuantity = Math.max(existingDrop.minQuantity, dropEntry.minQuantity);
											existingDrop.maxQuantity = Math.max(existingDrop.maxQuantity, dropEntry.maxQuantity);
										} else {
				...
				console.error(`Error processing loot template for ${creature.name}:`, error.message);
			}
		}
