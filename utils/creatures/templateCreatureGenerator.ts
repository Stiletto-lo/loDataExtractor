 ...
 */
function extractTierFromTemplateName(
	templateName?: string,
): string | undefined {
	if (!templateName) {
		return undefined;
 ...
 */
function extractCategoryFromTemplateName(templateName?: string): string {
	if (!templateName) {
		return "Structure";
	}
	...
	if (templateName.includes("Chest") || templateName.includes("Box"))
		return "Container";
...
function createCreaturesForOrphanedTemplates() {
	...
	console.info(
		`Found ${lootTemplates.length} loot templates and ${creatures.length} existing creatures`,
	);
	console.info(
		`${assignedTemplates.size} templates are already assigned to creatures`,
	);
	...
	console.info(
		`Created ${newCreatures.length} new creatures for orphaned templates`,
	);
	return newCreatures;
}
