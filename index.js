...
const processAndSaveItems = async () => {
	...
	await exportItemFiles();
};

const processTechData = async () => {
	let techData = fileParser.getTechData();
	techData = removeDuplicateTech(techData);
	techData.sort(orderByName);

	if (techData.length > 0) {
		await exportTechFiles(techData);
	}
};

const processCreatureData = async () => {
	...
	creatures = creatureProcessor.processCreatures(
		creatures,
		fileParser.getTranslator(),
		lootTables,
	);
	creatures.sort(orderByName);

	if (creatures.length > 0) {
		await exportCreatureFiles(creatures);
	}
};
